...
def midi_to_abc(filename=None, notes=None, key=None, metre=Fraction(3, 4), tempo = 180, default_len=Fraction(1, 16), bars_per_line=4, title='', source='', no_triplets=False, no_broken_rythms=False, slur_8th_pairs=False, slur_16th_pairs=False, slur_triplets=True, no_beam_breaks=False, index='1', anacrusis_notes=0):
    global num_quarter_notes_per_bar
    ...
    for track in midi_file.tracks:
        for msg in track:
            ...
            if msg.type == 'time_signature':
                time_signature = Fraction(msg.numerator, msg.denominator)
            elif msg.type == 'key_signature':
                key_signature = msg.key
            elif msg.type == 'set_tempo':
                default_tempo = mido.tempo2bpm(msg.tempo)

    return time_signature, key_signature, default_tempo

def my_midi2abc(file_name):
    default_len = Fraction(1, 16)
    metre, key, tempo = parse_midi(file_name)
    bars_per_line = 4
    title = ''
    source = ''
    no_triplets = False
    ...
    result = midi_to_abc(file_name, None, key, metre, int(tempo), default_len, bars_per_line, title, source,
                             no_triplets, no_broken_rythms, slur_8th_pairs, slur_16th_pairs, no_beam_breaks=no_beam_breaks)
    # print(result)
    return result
